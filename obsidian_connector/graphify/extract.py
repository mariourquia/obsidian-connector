...
# ── Obsidian Markdown extractor ───────────────────────────────────────────────

def extract_obsidian(path: Path) -> dict:
    import re
    stem = path.stem
    file_nid = _make_id(stem)
    str_path = str(path)
    ...
    # Regexes for wiki links and tags
    ...
    for i, line in enumerate(content.split("\n"), start=1):
        for m in wiki_re.finditer(line):
            ...
            page = inner.split("|")[0].split("#")[0].strip()
            if page:
                tgt_nid = _make_id(page)
                add_node(tgt_nid, page, i)
                add_edge(file_nid, tgt_nid, "references", i)

        for m in tag_re.finditer(line):
            tag = m.group(2)
            tgt_nid = _make_id("tag", tag)
            add_node(tgt_nid, f"#{tag}", i)
            add_edge(file_nid, tgt_nid, "tagged", i)
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from obsidian_connector.graphify.detect import _load_graphifyignore, _is_ignored
