...
try:
    ...
    # graphify ships as a submodule of obsidian-connector; read the host package's version.
    __version__ = _pkg_version("obsidian-connector")
except Exception:
    __version__ = "unknown"
...
- After modifying code files in this session, run `python3 -c "from obsidian_connector.graphify.watch import _rebuild_code; from pathlib import Path; _rebuild_code(Path('.'))"` to keep the graph current
"""
...
def main() -> None:
    ...
    if cmd == "install":
        ...
        from obsidian_connector.graphify.hooks import install as hook_install, uninstall as hook_uninstall, status as hook_status
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from obsidian_connector.graphify.serve import _score_nodes, _bfs, _dfs, _subgraph_to_text
        from obsidian_connector.graphify.security import sanitize_label
        ...
        from obsidian_connector.graphify.ingest import save_query_result as _sqr
        out = _sqr(
        ...
        from obsidian_connector.graphify.benchmark import run_benchmark, print_benchmark
