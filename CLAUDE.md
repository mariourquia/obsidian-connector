...
## Creation registry (canonical `sync_config.json`)

The Creation repo registry is the **single source of truth** shared by both
engines that project repo state into the Creation Vault: the Python/MCP engine
(`project_sync.load_sync_config`, the `obsx creation` dashboard read-layer) and
the bash nightly engine (`~/.local/bin/sync-creation-vault`, which reads it via
`jq`). It lists every tracked repo (dir, display, status, group, tags), a
`groups` slug→display map, and `github_root`.

- **Resolution** (`project_sync.resolve_sync_config_path`, most specific first):
  `$OBSIDIAN_SYNC_CONFIG` → `<vault root>/sync_config.json` → XDG
  `~/.config/obsidian-connector/sync_config.json` (canonical home) →
  `discover_repos()` groupless fallback. Canonical home is non-iCloud
  (no eviction) and non-public (the real registry lists private repos).
- **Grouping**: `creation_projects.list_projects` collapses non-`standalone`
  repos sharing a `group` into one Project, naming it from
  `SyncConfig.group_display_names` (the `groups` map) with the built-in
  `GROUP_DISPLAY` as fallback. Result: 57 standalone → 19 Projects (MCMC = 9).
- **Vault default**: `obsx creation` + creation MCP tools default to the vault
  named `creation` (`config.CREATION_VAULT_NAME`), `--vault` overrides.
- Schema/example: `creation/README.md`, `creation/sync_config.example.json`.
  ADR: `docs/architecture/creation-registry-canonical.md`. The bash engine
  fails loudly (missing `jq` / absent / invalid / empty registry) rather than
  syncing groupless.
...
- `config.py` -- vault/index configuration, vault path resolution. `CREATION_VAULT_NAME = "creation"` is the default vault for `obsx creation` / creation MCP tools.
- `project_sync.py` -- repo-sync engine + canonical registry loader: `resolve_sync_config_path` / `xdg_sync_config_path` (env → vault-root → XDG), `load_sync_config` (parses `groups` into `SyncConfig.group_display_names`), `SYNC_CONFIG_ENV = "OBSIDIAN_SYNC_CONFIG"`. See ADR `docs/architecture/creation-registry-canonical.md`.
- `creation_projects.py` -- derives `Project`s from the registry by group; prefers config `group_display_names` over the built-in `GROUP_DISPLAY`.
